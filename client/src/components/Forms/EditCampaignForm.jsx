...
import { Editing, money } from "../../assets";
...
const EditCampaignForm = () => {
  ...
  const selectedImage = () => {
    ...
    if (file) {
      ...
      selected = state.picture;
    }
    return selected;
  };

  const onSubmit = async (data) => {
    data.posterPic = state.posterPic;
    data.posterName = state.posterName;
    data.postedBy = state.postedBy;
    ...
    if (!picture) {
      data.picture = state.picture;
      // console.log("EDIT DATA(No Pic): ", data);
      formData.append("picture", data.picture);
      if (ethers.utils.isAddress(data.walletAddress)) {
        editCampaign(formData, state._id);
      } else {
      ...
      // console.log("EDIT DATA(Pic): ", data);
      formData.append("picture", data.picture);
      if (ethers.utils.isAddress(data.walletAddress)) {
        editCampaign(formData, state._id);
      } else {
